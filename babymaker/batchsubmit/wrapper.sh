...
# run it
#
echo "[wrapper] running: python do.py test ${INDIR} ${FILEID} output"

python do.py test ${INDIR} ${FILEID} output
...
OUTPUT=`ls output_files/test/*.root`
...
if [ -f output.root ]; then
    echo "Skimming..."
    python copyTree.py output.root skimmed.root -1 0 "Events" "nmcand > 0"
fi
...
gfal-copy -p -f -t 4200 --verbose file://`pwd`/skimmed.root gsiftp://gftp.t2.ucsd.edu${COPYDIR}/${FILEID}.root
